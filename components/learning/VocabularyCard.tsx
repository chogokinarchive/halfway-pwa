  ...
  return (
    <Card className="flex flex-col gap-3 p-4">
      ...
      <div className="space-y-1">
        <p className="text-xl font-semibold tracking-tight">{item.italian}</p>
        <p className="text-lg">{item.japanese}</p>
        {item.reading && <p className="text-sm text-muted-foreground">{item.reading}</p>}
      </div>

      <button
        onClick={reveal}
        className="rounded-lg border border-dashed border-border px-3 py-2 text-left text-sm text-muted-foreground transition-colors hover:bg-accent"
      >
        {revealed ? (
          <motion.span
            initial={{ opacity: 0 }}
